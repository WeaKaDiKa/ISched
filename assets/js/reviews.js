...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addBtn.addEventListener('click', () => {
    ...
    selectedRating = 0;
    resetStars();
    // Auto-select multiple services if preselectServices is set
    if (window.preselectServices && Array.isArray(window.preselectServices) && window.preselectServices.length > 0) {
      for (let i = 0; i < serviceSel.options.length; i++) {
        if (serviceSel.options[i].value !== "") {
          serviceSel.options[i].selected = true; // auto-select all visible options
        } else {
          serviceSel.options[i].selected = false;
        }
      }
      serviceSel.disabled = false; // allow user to unselect if gusto nya
    } else {
  ...
  submitBtn.addEventListener('click', () => {
    ...
    const services = Array.from(serviceSel.selectedOptions)
      .map(opt => opt.value)
      .filter(val => val !== "");
    const text    = reviewText.value.trim();

    if (!services.length || !selectedRating || !text) {
      return alert('Please select at least one service, rating, and enter feedback.');
    }
