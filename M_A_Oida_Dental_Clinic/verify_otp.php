...
try {
    ...
    if ($result->num_rows !== 1) {
        error_log("No matching OTP found for email: " . $email);
        throw new Exception('Invalid or expired OTP code');
    }

    $patient = $result->fetch_assoc();
    error_log("OTP Details - Expires at: " . $patient['otp_expires'] . ", Seconds left: " . $patient['seconds_left']);

    // Check if OTP is expired (negative seconds means expired)
    if ($patient['seconds_left'] <= 0) {
        error_log("OTP expired. Expiry time was: " . $patient['otp_expires'] . ", Current server time: " . date('Y-m-d H:i:s'));
    ...
    try {
        ...
        if (!$stmt->execute()) {
            error_log("Failed to insert into patients: " . $stmt->error);
