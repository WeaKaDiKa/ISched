...
<body class="bg-blue-100 min-h-screen flex items-center justify-center">
    <div class="flex flex-col md:flex-row w-full min-h-screen items-center justify-center">
        ...
        <div class="flex justify-center items-center w-full md:w-1/2 p-8">
            <div class="w-full max-w-md mx-auto bg-white rounded-2xl shadow-xl p-10 border border-blue-300">
                ...
                <?php elseif ($success): ?>
                    <p class="bg-green-100 text-green-700 px-4 py-2 rounded mb-4 text-sm text-center"><?= htmlspecialchars($success) ?></p>
                <?php endif; ?>
                <form action="admin_signup.php" method="POST" class="space-y-6">
                    <div>
                        <label for="first_name" class="block text-sm font-medium text-gray-700 mb-1">First Name <span class="text-red-500">*</span></label>
                        <input type="text" name="first_name" id="first_name" value="<?= htmlspecialchars($_POST['first_name'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your first name" />
                    </div>
                    <div>
                        <label for="last_name" class="block text-sm font-medium text-gray-700 mb-1">Last Name <span class="text-red-500">*</span></label>
                        <input type="text" name="last_name" id="last_name" value="<?= htmlspecialchars($_POST['last_name'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your last name" />
                    </div>
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Full Name <span class="text-red-500">*</span></label>
                        <input type="text" name="name" id="name" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your full name" />
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Your email <span class="text-red-500">*</span></label>
                        <input type="email" name="email" id="email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your email" />
                    </div>
                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Your password <span class="text-red-500">*</span></label>
                        <input type="password" name="password" id="password" value="<?= htmlspecialchars($_POST['password'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter password" />
                    ...
                    <div>
                        ...
                        <input type="number" name="age" id="age" value="<?= htmlspecialchars($_POST['age'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your age" />
                    </div>
                    <div>
                        <label for="mobile" class="block text-sm font-medium text-gray-700 mb-1">Mobile Number <span class="text-red-500">*</span></label>
                        <input type="text" name="mobile" id="mobile" value="<?= htmlspecialchars($_POST['mobile'] ?? '') ?>" required class="block w-full border border-gray-300 rounded-lg bg-gray-50 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Enter your mobile number" />
                    </div>
                    <div>
                    ...
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded-lg shadow-sm transition">Signup</button>
                    <button type="button" onclick="window.location.href='admin_login.php'" class="w-full mt-3 bg-gray-200 text-gray-800 font-semibold py-2 rounded-lg shadow-sm transition cursor-pointer">Already have an account? Login</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div id="successModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50 hidden">
        <div class="bg-white rounded-xl shadow-lg p-8 max-w-xs w-full text-center">
            ...
            <button onclick="redirectToRequestAccess()" class="bg-green-500 text-white font-semibold py-2 px-4 rounded">OK</button>
        </div>
    </div>
</body>
</html>