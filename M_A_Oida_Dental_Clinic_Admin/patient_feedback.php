...
<body class="bg-white text-gray-900">
                        ...
                        <table class="min-w-full text-xs text-left border-collapse">
                            ...
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr class="border-b hover:bg-blue-50">
                                    ...
                                    <td class="px-4 py-2">
                                        <?php for($i=0; $i<$row['rating']; $i++) echo '<i class="fas fa-star text-yellow-400"></i>'; ?>
                                        <?php for($i=$row['rating']; $i<5; $i++) echo '<i class="far fa-star text-gray-300"></i>'; ?>
                                    </td>
                                    <td class="px-4 py-2 text-gray-700 max-w-xs truncate" title="<?= htmlspecialchars($row['text']) ?>">
    ...
    <div id="feedbackSuccessModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden transition-opacity duration-300">
      <div class="bg-white rounded-xl shadow-lg p-8 max-w-sm w-full text-center relative transform transition-all duration-300 scale-90 opacity-0">
        ...
        <button onclick="closeFeedbackSuccessModal()" class="w-full bg-green-400 text-white font-semibold py-2 rounded hover:bg-green-500 transition">OK</button>
      </div>
    </div>
